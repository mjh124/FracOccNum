...
#        print bra[i], ket[i]
    ...
    for p in range(Nbasis):
        for q in range(Nbasis):
            ...
            Dpq = 0
            bra_cp = copy.deepcopy(sigBraPairs)
            for i in bra_cp:
                i_cp = copy.deepcopy(i)
                i_key = i_cp[0]
                i_amp = ONstring2Amp[i_key]

                i_array = i_cp[1]
                ket_cp = copy.deepcopy(sigKetPairs)
                for j in ket_cp:

                    if ONstring2Amp[j[0]]*i_amp <= 1e-10:
                        continue

                    j_cp = copy.deepcopy(j)
                    j_key = j_cp[0]
                    j_array = j_cp[1] 
			...
			for k in range(0, p):
                        ...
                        #    if i_array[k] != j_array[k]:
                        #        ON_overlap = 0

                        #Bring everything together
#                        dij = delta_p * delta_q * gamma_p * gamma_q * ON_overlap #This is true for p > q
                        dij = delta_p * delta_q * gamma_p * gamma_q * ON_overlap * i_amp * ONstring2Amp[j_key]
                        if dij != 0:
                    ...
                    #print "D[%d][%d], Contribution from ON %s, to ON %s, dij = %s" % (p, q, i_cp, j_cp, dij)
                        ON_overlap = np.multiply(i_array, j_array)
                    #print "  %d" % (dij)
                    #Dpq += dij
                    Dpq += abs(dij)
