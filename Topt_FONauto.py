...
if len(sys.argv) != 9:
        print "Usage: Topt_FON_unres.py qchem-input #AS tot_a_orbs tot_b_orbs aHOMO bHOMO aTemps bTemps"
        print "Temps are inserted as a comma separated string of temps"
        exit(0)
...
def get_Temps(aTemps, bTemps):

    aT = re.split(",", aTemps)
    bT = re.split(",", bTemps)
    temps = []
    for i in range(len(aT)):
        temps.append(float(aT[i]))
        temps.append(float(bT[i]))
    return temps
...
def main():
    ...
    temps = get_Temps(aTemps, bTemps)

    occ_aidx, virt_alpha = get_alpha_args(num_AS, num_aOrbs, aHOMO)
    occ_bidx, virt_beta = get_beta_args(num_AS, num_bOrbs, bHOMO)

    res = minimize(energy, temps, args=(num_AS, num_aOrbs, num_bOrbs, occ_aidx, occ_bidx, virt_alpha, virt_beta, filename), jac=None, method='nelder-mead', options={'disp':True, 'xtol':1, 'maxiter':100000}, callback=print_energy)
